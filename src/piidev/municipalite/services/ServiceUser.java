...
Connection cnx = MyConnection.getInstance().getCnx();
    ...
    public void modifier(User u) {
     ...
     public void modifierpwd(User u) {
       try {
        // Hash le nouveau mot de passe à l'aide de l'algorithme BCrypt
        String hashedPassword = BCrypt.hashpw(u.getPassword(), BCrypt.gensalt(12));

        // Met à jour le mot de passe hashé dans la base de données
        String updatereq = "UPDATE user SET password = ? WHERE id = ?";
        PreparedStatement pr2 = cnx.prepareStatement(updatereq);
        pr2.setString(1, hashedPassword);
        pr2.setInt(2, u.getId());
        pr2.executeUpdate();
